...
test.afterAll(async () => {
  ...
  await client.execute(sql`DELETE FROM subscribers WHERE email LIKE 'it-%@example.com'`);
});

test.describe('Frontend test', ()=>{
test('Displays page UI', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'Mystery Page' })).toBeVisible();
  await expect(page.getByLabel('Enter your Email')).toBeVisible();
  await expect(page.getByRole('button', { name: 'I want to know!'})).toBeVisible();
});
});
...
(run ? test.describe : test.describe.skip)('Frontend + Backend tests', () => {
  test('Subscribe new mail -> success message', async ({ page }) => {
    ...
    await expect(page.getByText('Successfully subscribed')).toBeVisible({ timeout: 10000 });
